@if (@isset($data) && !@empty($data) && count($data) > 0)
            ...
            @foreach ($data as $d)
                <tr>
                    ...
                    <td style=""class="control">
                        <div style="">
                            <a href="{{ route('admin.suppliers.show', [$d->id]) }}" id=""
                                class="btn btn-sm btn-primary " style="margin-left: 5px">عرض التفاصيل</a>

                            <a href="{{ route('admin.suppliers.edit', [$d->id]) }}" class="btn btn-sm btn-success"
                                style="margin-left: 5px"> تعديل
                            </a>

                            <a href="{{ route('admin.suppliers.destroy', [$d->id]) }}" id="are_you_sure"
    ...
    <div class="col-md-12" id="ajax_pagination_suppliers_search">
        {{ $data->links() }}
    </div>
@else
