...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card" style="overflow-x:auto;background:rgb(3, 47, 53);color:aliceblue">
                <div class="card-header" style="background:rgb(247, 247, 250);">
                    <h3 class="card-title card_title_center" style="font-size: 30px;font-family: ;color:rgb(30, 15, 248)">
                        إضافة فاتورة مشتريات
                    </h3>
                </div>
                <div class="card-body" style="overflow-x:auto;background:rgb(3, 47, 53)">
                    <form action="{{ route('admin.suppliers_with_order.store') }}" method="post" autocomplete="off">
                        {{--  {{ method_field('patch') }}  --}}
                        {{ csrf_field() }}
                        <div class="row">
                            <div class="col-md-4">
                                <label for="DOC_NO" class="control-label"> رقم الفاتورة المسجل بأصل المشتريات :</label>
                                <input type="number" class="form-control " id="DOC_NO" name="DOC_NO"
                                    value="{{ old('DOC_NO') }}" placeholder="رقم الفاتورة   ">
                                @error('DOC_NO')
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="supplier_code" id="supplier_code" class="form-control select2 ">
                                    <option selected style="text-align: center" value="">-- حــــــدد اسم المورد
                                        --</option>
                                    @if (isset($suppliers) && !@empty($suppliers))
                                        @foreach ($suppliers as $sup)
                                            <option @if (old('supplier_code') == $sup->supplier_code) selected="selected" @endif
                            ...
                            <div class="col-md-4 ">
                                <div class="form-group">
                                    ...
                                    <select name="store_id" id="store_id" class="form-control select2 ">
                                        <option selected style="text-align: center" value="">-- حــــــدد اسم المخزن
                                            المستلم للفاتورة
                                            --</option>
                                        @if (isset($stores) && !@empty($stores))
                                            @foreach ($stores as $str)
                                                <option @if (old('store_id') == $str->id) selected="selected" @endif
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="bill_type" id="bill_type" class="form-control">
                                    <option selected style="text-align: center" value=""> --اختــــــر نـــــوع
                                        الفاتورة-- </option>
                                    <option class="bg-primary" @if (old('bill_type') == 1) selected="selected" @endif
                                        value="1">
                                        كــــاش </option>
                                    <option class="bg-warning" @if (old('bill_type') == 2) selected="selected" @endif
                                        value="2">
                                        آجــــل </option>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="is_provide" id="is_provide" class="form-control">
                                    <option selected style="text-align: center" value=""> --اختــــــــر حـــــــــالة
                                        الاعتماد-- </option>
                                    <option @if (old('is_provide') == 0 and old('is_provide') != '') selected="selected" @endif value="0">
                                        لا </option>
                                    <option @if (old('is_provide') == 1) selected="selected" @endif value="1">
                                        نعم </option>
                                </select>
                                @error('is_provide')
                                    <span class="text-danger">{{ $message }}</span>
